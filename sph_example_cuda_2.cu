...
__global__ void kernelDeriv0Global(double* r, double h, int n, double* weights) {
...
__global__ void densityKernel(double* x, double* rho, double m, double h, int n) {
    ...
    kernelDeriv0Global<<<gridSize, blockSize>>>(uij.data(), h, n, weights);
    cudaDeviceSynchronize()
    VectorXd weightsVec = Map<VectorXd>(weights, n);

    VectorXd rho_ij = m * weightsVec;
    rho[i] = rho_ij.sum();
